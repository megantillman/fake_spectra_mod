...
class Profiles(object):
    ...
    def do_fit(self, tol=tol, signif=signif, sattau=4):
        """Do the fit.
        ...
        result = optimize.minimize(self.fun_min_multiple, inputs, tol=realtol/tol*0.25, method='L-BFGS-B')
        ...
        for maxpk in range(3, np.size(self.stddev)+1):
            ...
            new_result = optimize.minimize(self.fun_min_multiple, inputs, tol=realtol/tol*0.25, method='L-BFGS-B')
...
#         assert np.all(np.abs((tol+refitted)/(tol+self.tau) -1.) < 0.5)
    ...
    def fun_min(self, stddev, amplitude, tau):
        ...
        wasserstein_dis = EMD(np.exp(-tau[minn:maxx]), np.exp(-gauss[minn:maxx]))
    ...
    def fun_min_multiple(self, inputs):
        ...
        return EMD(np.exp(-self.tau), np.exp(-gauss))
...
def get_b_param_dist(taus, dvbin, elem="H", ion=1, line=1215):
    ...
    newresult = optimize.minimize(_opt_power_fit, result, args=(lb_params, ln_vals), tol=tol).x
